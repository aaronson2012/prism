...
async def test_shutdown_includes_cleanup_delay():
    ...
    with patch('prism.main.load_config') as mock_load_config, \
        ...
        # The sleep should be called with 0.25 seconds
        sleep_calls = [call for call in mock_sleep.call_args_list if call[0][0] == 0.25]
        assert len(sleep_calls) >= 1, "Expected asyncio.sleep(0.25) to be called for cleanup delay"
